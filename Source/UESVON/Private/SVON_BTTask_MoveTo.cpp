...
USVON_BTTask_MoveTo::USVON_BTTask_MoveTo(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	BlackboardKey.AddObjectFilter(this, GET_MEMBER_NAME_CHECKED(USVON_BTTask_MoveTo, BlackboardKey), AActor::StaticClass());
	BlackboardKey.AddVectorFilter(this, GET_MEMBER_NAME_CHECKED(USVON_BTTask_MoveTo, BlackboardKey));
}

EBTNodeResult::Type USVON_BTTask_MoveTo::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (NodeResult == EBTNodeResult::InProgress && bObserveBlackboardValue)
	{
		...
		if (ensure(BlackboardComp))
		{
			...
			MyMemory->BBObserverDelegateHandle = BlackboardComp->RegisterObserver(BlackboardKey.GetSelectedKeyID(), this, FOnBlackboardChangeNotification::CreateUObject(this, &USVON_BTTask_MoveTo::OnBlackboardValueChange));
		}
	}
...
EBTNodeResult::Type USVON_BTTask_MoveTo::PerformMoveTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (MyController && MyBlackboard)
	{
		...
		if (BlackboardKey.SelectedKeyType == UBlackboardKeyType_Object::StaticClass())
		{
			...
			{
				UE_VLOG(MyController, LogBehaviorTree, Warning, TEXT("USVON_BTTask_MoveTo::ExecuteTask tried to go to actor while BB %s entry was empty"), *BlackboardKey.SelectedKeyName.ToString());
			}
		}
...
UAITask_MoveTo* USVON_BTTask_MoveTo::PrepareMoveTask(UBehaviorTreeComponent& OwnerComp, UAITask_MoveTo* ExistingTask, FAIMoveRequest& MoveRequest)
{
...
EBlackboardNotificationResult USVON_BTTask_MoveTo::OnBlackboardValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
EBTNodeResult::Type USVON_BTTask_MoveTo::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	FBTMoveToTaskMemory* MyMemory = reinterpret_cast<FBTMoveToTaskMemory*>(NodeMemory);
	if (!MyMemory->bWaitingForPath)
	{
...
void USVON_BTTask_MoveTo::OnTaskFinished(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTNodeResult::Type TaskResult)
{
	FBTMoveToTaskMemory* MyMemory = reinterpret_cast<FBTMoveToTaskMemory*>(NodeMemory);
	MyMemory->Task.Reset();
...
void USVON_BTTask_MoveTo::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
...
void USVON_BTTask_MoveTo::OnMessage(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, FName Message, int32 SenderID, bool bSuccess)
{
...
void USVON_BTTask_MoveTo::OnGameplayTaskActivated(UGameplayTask& Task)
{
	
}

void USVON_BTTask_MoveTo::OnGameplayTaskDeactivated(UGameplayTask& Task)
{
...
FString USVON_BTTask_MoveTo::GetStaticDescription() const
{
...
void USVON_BTTask_MoveTo::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
uint16 USVON_BTTask_MoveTo::GetInstanceMemorySize() const
{
	return sizeof(FBTMoveToTaskMemory);
}

void USVON_BTTask_MoveTo::PostLoad()
{
	Super::PostLoad();
}
...
FName USVON_BTTask_MoveTo::GetNodeIconName() const
{
	return FName("BTEditor.Graph.BTNode.Task.MoveTo.Icon");
}

void USVON_BTTask_MoveTo::OnNodeCreated()
{
