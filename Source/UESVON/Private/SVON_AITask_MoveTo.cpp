...
USVON_AITask_MoveTo::USVON_AITask_MoveTo(const FObjectInitializer& ObjectInitializer)
...
USVON_AITask_MoveTo* USVON_AITask_MoveTo::AIMoveTo(AAIController* Controller, FVector InGoalLocation, AActor* InGoalActor,
...
{
	USVON_AITask_MoveTo* MyTask = Controller ? UAITask::NewAITask<USVON_AITask_MoveTo>(*Controller, EAITaskPriority::High) : nullptr;
	if (MyTask)
	{
...
void USVON_AITask_MoveTo::SetUp(AAIController* Controller, const FAIMoveRequest& InMoveRequest)
{
	OwnerController = Controller;
	MoveRequest = InMoveRequest;
}

void USVON_AITask_MoveTo::SetContinuousGoalTracking(bool bEnable)
{
	bUseContinuousTracking = bEnable;
}

void USVON_AITask_MoveTo::FinishMoveTask(EPathFollowingResult::Type InResult)
{
...
void USVON_AITask_MoveTo::Activate()
{
	Super::Activate();
...
void USVON_AITask_MoveTo::ConditionalPerformMove()
{
	if (MoveRequest.IsUsingPathfinding() && OwnerController && OwnerController->ShouldPostponePathUpdates())
	{
		...
		OwnerController->GetWorldTimerManager().SetTimer(MoveRetryTimerHandle, this, &USVON_AITask_MoveTo::ConditionalPerformMove, 0.2f, false);
	}
	else
	{
...
void USVON_AITask_MoveTo::PerformMove()
{
	UPathFollowingComponent* PFComp = OwnerController ? OwnerController->GetPathFollowingComponent() : nullptr;
	if (PFComp == nullptr)
	{
	...
	case EPathFollowingRequestResult::RequestSuccessful:
		MoveRequestID = ResultData.MoveId;
		PathFinishDelegateHandle = PFComp->OnRequestFinished.AddUObject(this, &USVON_AITask_MoveTo::OnRequestFinished);
...
void USVON_AITask_MoveTo::Pause()
{
...
void USVON_AITask_MoveTo::Resume()
{
	Super::Resume();
...
void USVON_AITask_MoveTo::SetObservedPath(FNavPathSharedPtr InPath)
{
	...
	if (Path.IsValid())
	{
		...
		PathUpdateDelegateHandle = Path->AddObserver(FNavigationPath::FPathObserverDelegate::FDelegate::CreateUObject(this, &USVON_AITask_MoveTo::OnPathEvent));
	}
}

void USVON_AITask_MoveTo::ResetObservers()
{
	if (Path.IsValid())
	{
...
void USVON_AITask_MoveTo::ResetTimers()
{
...
void USVON_AITask_MoveTo::OnDestroy(bool bInOwnerFinished)
{
...
void USVON_AITask_MoveTo::OnRequestFinished(FAIRequestID RequestID, const FPathFollowingResult& Result)
{
	if (RequestID == MoveRequestID)
	{
		...
		else
		{
			...
			if (bUseContinuousTracking && MoveRequest.IsMoveToActorRequest() && Result.IsSuccess())
			{
				...
				GetWorld()->GetTimerManager().SetTimerForNextTick(this, &USVON_AITask_MoveTo::PerformMove);
			}
			else
			{
...
void USVON_AITask_MoveTo::OnPathEvent(FNavigationPath* InPath, ENavPathEvent::Type Event)
{
...
void USVON_AITask_MoveTo::ConditionalUpdatePath()
{
	...
	{
		UE_VLOG(GetGameplayTasksComponent(), LogGameplayTasks, Log, TEXT("%s> can't path right now, waiting..."), *GetName());
		OwnerController->GetWorldTimerManager().SetTimer(PathRetryTimerHandle, this, &USVON_AITask_MoveTo::ConditionalUpdatePath, 0.2f, false);
	}
	else
	{
		PathRetryTimerHandle.Invalidate();
